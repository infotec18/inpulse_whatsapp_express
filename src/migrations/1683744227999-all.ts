...
export class All1683744227999 implements MigrationInterface {
    name = 'All1683744227999'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`mensagens\` (\`CODIGO\` int NOT NULL AUTO_INCREMENT, \`CODIGO_ATENDIMENTO\` int NOT NULL, \`CODIGO_OPERADOR\` int NOT NULL, \`TIPO\` varchar(30) NOT NULL, \`MENSAGEM\` text NULL, \`FROM_ME\` tinyint NOT NULL, \`DATA_HORA\` datetime NOT NULL, \`TIMESTAMP\` bigint NOT NULL, \`ID\` varchar(255) NOT NULL, \`ID_REFERENCIA\` varchar(255) NULL, PRIMARY KEY (\`CODIGO\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`atendimentos\` (\`CODIGO\` int NOT NULL AUTO_INCREMENT, \`ATIVO_RECEP\` enum('ATIVO', 'RECEP') NOT NULL, \`CODIGO_OPERADOR\` int NOT NULL, \`CODIGO_OPERADOR_ANTERIOR\` int NOT NULL, \`CODIGO_CLIENTE\` int NOT NULL, \`CODIGO_NUMERO\` int NOT NULL, \`CONCLUIDO\` tinyint NULL DEFAULT '0', \`DATA_INICIO\` datetime NOT NULL, \`DATA_FIM\` datetime NULL, \`DATA_AGENDAMENTO\` datetime NULL, \`URGENCIA_SUPERVISOR\` enum ('URGENTE', 'MUITO_ALTA', 'ALTA', 'MEDIA', 'NORMAL') NULL, \`URGENCIA_AGENDAMENTO\` enum ('MUITO_ALTA', 'ALTA', 'MEDIA', 'NORMAL') NULL, \`URGENCIA_OPERADOR\` enum ('ALTA', 'MEDIA', 'NORMAL') NOT NULL DEFAULT 'NORMAL', PRIMARY KEY (\`CODIGO\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`mensagens\` ADD CONSTRAINT \`FK_e4bf0a53dc4657e2b1a87cb4f80\` FOREIGN KEY (\`CODIGO_OPERADOR\`) REFERENCES \`operadores\`(\`CODIGO\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`clientes_numeros\` ADD CONSTRAINT \`FK_68d2b0bd18f11ff5d41049e2664\` FOREIGN KEY (\`CODIGO_CLIENTE\`) REFERENCES \`clientes\`(\`CODIGO\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
