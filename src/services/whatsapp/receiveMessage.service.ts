...
export async function receiveWhatsappMessageService (data: any) {
    ...
    if (attending) {
        const newMessage = await services.messages.createOficial(message, attending.CODIGO_ATENDIMENTO, attending.CODIGO_OPERADOR);
            ...
            if(findAttendance && findCustomer) {
                ...
                const avaliableOperator = await services.attendances.getOperator(findCustomer.OPERADOR);
                    ...
                    if(typeof avaliableOperator === "number") {
                        ...
                        const newMessage = await services.messages.createOficial(message, newAttendance.CODIGO, newAttendance.CODIGO_OPERADOR);
