...
/* export async function findByOperatorIdService(CODIGO_OPERADOR: number) {
    ...
    const findInClientsCampaign = await clientsCampaignRepository.findBy({
        CONCLUIDO: "NAO", OPERADOR: CODIGO_OPERADOR
    });

    const clientIds = findInClientsCampaign.map(cc => cc.CLIENTE);

    const findNumbers = await clientsNumbersRepository
        .createQueryBuilder("clientes_numeros")
        .where("clientes_numeros.CODIGO_CLIENTE in (:...ids)", { ids: clientIds })
        .getMany();

    let returnArr = [];

    findNumbers.forEach(async(n) => {
        const findCustomer = await customersRepository.findOneBy({ CODIGO: n.CODIGO_CLIENTE });

        if(findCustomer) returnArr.push({
                            CODIGO_CLIENTE: n.CODIGO_CLIENTE,
                            CODIGO_NUMERO: n.CODIGO,
                            NOME: n.NOME,
                            RAZAO: findCustomer.RAZAO,
